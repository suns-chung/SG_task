...
workflow PreProcessingForVariantDiscovery_GATK4 {
  ...
  String gatk_docker = select_first([gatk_docker_override, "hmsrc-gatk"])
...
task SamToFastqAndBwaMem {
  ...
  command <<<
    ...
    bash_ref_fasta=${ref_fasta}
		...
		java -XX:+UseSerialGC -Dsamjdk.compression_level=${compression_level} ${java_opt} -jar /n/app/picard/2.8.0/bin/picard-2.8.0.jar \
      SamToFastq \
			INPUT=${input_bam} \
			FASTQ=/dev/stdout \
			INTERLEAVE=true \
...
task MergeBamAlignment {
  ...
  command {
    # set the bash variable needed for the command-line
    bash_ref_fasta=${ref_fasta}
    ${gatk_path} --java-options "-XX:+UseSerialGC -Dsamjdk.compression_level=${compression_level} ${java_opt}" \
      MergeBamAlignment \
...
task SortAndFixTags {
  ...
  command {
    ...
    ${gatk_path} --java-options "-XX:+UseSerialGC -Dsamjdk.compression_level=${compression_level} ${java_opt_sort}" \
      SortSam \
      --INPUT ${input_bam} \
      --OUTPUT /dev/stdout \
    ...
    | \
    ${gatk_path} --java-options "-XX:+UseSerialGC -Dsamjdk.compression_level=${compression_level} ${java_opt_fix}" \
      SetNmMdAndUqTags \
      --INPUT /dev/stdin \
      --OUTPUT ${output_bam_basename}.bam \
      --CREATE_INDEX true \
...
task MarkDuplicates {
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  ...
  runtime {
    ...
    singularity: docker_image
    memory: mem_size
    requested_memory_mb_per_core: 48000
    disks: "local-disk " + disk_size + " HDD"
  }
  output {
    File output_bam = "${output_bam_basename}.bam"
...
task BaseRecalibrator {
  ...
  command { 
    ${gatk_path} --java-options "-XX:+UseSerialGC ${java_opt}" \
      BaseRecalibrator \
      -R ${ref_fasta} \
      -I ${input_bam} \
